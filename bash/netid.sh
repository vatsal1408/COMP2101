...
# For Task 2 I used ip link show command to display interface name and I also sorted the output using awk and grep command
 

  
  for(( i =1; i < 10; i++)); do
   echo " The list of $i interface is mentioned below"
    ip link show | grep $i: | awk '{print $1 $2}'
  done
